...
void CLocalConnection::CaptureState(
	...
	LOG("[LocalConn::%s] captured queues: instances=%u q0=%u q1=%u",
		__func__,
		outNumInstances,
		static_cast<unsigned int>(outQueue0.size()),
		static_cast<unsigned int>(outQueue1.size())
	);
}
...
void CLocalConnection::ApplyPendingRestore()
{
	...
	LOG("[LocalConn::%s] restored queues: instances=%u q0=%u q1=%u",
		__func__,
		restoreNumInstances,
		static_cast<unsigned int>(restoreQueues[0].size()),
		static_cast<unsigned int>(restoreQueues[1].size())
	);
